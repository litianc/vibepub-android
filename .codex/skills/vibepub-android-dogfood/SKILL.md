...
1. Inspect current repo and device state:
   ...
   adb devices -l
   ```
2. Prefer stable-signed local build for fast iteration:
   ```bash
...
- Always pass `--serial` when multiple `adb devices -l` rows refer to the same Xiaomi/Redmi device through mDNS and direct IP.
