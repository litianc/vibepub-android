...
function durableEnv(results: FakeBucket | undefined = bucket(), overrides: Record<string, unknown> = {}): any {
  ...
  return { ...base, VISUAL_OPERATION: new FakeOperationNamespace(base) };
}
...
function imageBody(operationId: string, attempt = 1): string {
  return JSON.stringify({ operation_id: operationId, attempt, model: "gpt-image-2", prompt: "synthetic prompt", size: "1536x864" });
}
