...
test("protected canary workflow always restores main before image and proves flag-off", () => {
  ...
  assert.match(workflow, /CANARY_DEPLOYMENT_MARKER=.*GITHUB_RUN_ATTEMPT.*https-image/);
  assert.match(workflow, /CANARY_IMAGE_DEPLOYED_AT=.*toISOString/);
  assert.match(workflow, /CANARY_MAIN_DEPLOYED_AT=.*toISOString/);
  assert.match(workflow, /--var "DEPLOYMENT_MARKER:\$\{CANARY_DEPLOYMENT_MARKER\}"/);
  assert.match(workflow, /wait-for-staging-health\.mjs/);
  assert.match(workflow, /--expected-main-deployed-at "\$CANARY_MAIN_DEPLOYED_AT"/);
  ...
  assert.match(workflow, /--required-adapters image/);
  ...
  const deploymentProof = workflow.indexOf("Prove the attested origin serves this exact reviewed deployment");
  const miningStart = workflow.indexOf("Start exactly the prepared Mining V3 handoff");
  assert.ok(deploymentProof > 0 && miningStart > deploymentProof);
  assert.match(workflow.slice(miningStart, workflow.indexOf("Poll the immutable publication projection", miningStart)), /test -s "\$RUNNER_TEMP\/canary-health\.json"/);
